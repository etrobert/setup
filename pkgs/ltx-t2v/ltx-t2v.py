...
stage 1 at half resolution (8 steps), x2 latent upscale, stage 2 refine (3 steps),
joint audio-video latents. Prompt enhancer omitted.
"""
import argparse, json, time, urllib.request

p = argparse.ArgumentParser()
p.add_argument("prompt")
p.add_argument("--negative", default="pc game, console game, video game, cartoon, childish, ugly")
p.add_argument("--width", type=int, default=1152)
p.add_argument("--height", type=int, default=640)
p.add_argument("--duration", type=int, default=5, help="seconds")
p.add_argument("--fps", type=int, default=24)
p.add_argument("--seed", type=int, default=1234)
p.add_argument("--transformer", default="LTX-2.5-Distilled-Q4_K_S.gguf", help="checkpoint in models/diffusion_models; loader inferred from extension")
...
loader = ({"class_type": "UnetLoaderGGUF", "inputs": {"unet_name": a.transformer}} if a.transformer.endswith(".gguf")
          else {"class_type": "UNETLoader", "inputs": {"unet_name": a.transformer, "weight_dtype": "default"}})

g = {
 "unet": loader,
 "clip": {"class_type": "CLIPLoader", "inputs": {"clip_name": "gemma4-12b-with-proj-ltx-2.5-comfy-int8-convrot.safetensors", "type": "ltxv", "device": a.te_device}},
 "vvae": {"class_type": "VAELoader", "inputs": {"vae_name": "ltx-2.5-video-vae-bf16.safetensors"}},
 "avae": {"class_type": "VAELoader", "inputs": {"vae_name": "ltx-2.5-audio-vae-bf16.safetensors"}},
 "upmodel": {"class_type": "LatentUpscaleModelLoader", "inputs": {"model_name": "ltx-2.5-latent-spatial-upscaler-x2-bf16-1.0.safetensors"}},
 ...
 "cond": {"class_type": "LTXVConditioning", "inputs": {"positive": ["pos", 0], "negative": ["neg", 0], "frame_rate": a.fps}},
 "vlat": {"class_type": "EmptyLTXVLatentVideo", "inputs": {"width": a.width // 2, "height": a.height // 2, "length": frames, "batch_size": 1}},
 "alat": {"class_type": "LTXVEmptyLatentAudio", "inputs": {"frames_number": frames, "frame_rate": a.fps, "batch_size": 1, "audio_vae": ["avae", 0]}},
 "cat1": {"class_type": "LTXVConcatAVLatent", "inputs": {"video_latent": ["vlat", 0], "audio_latent": ["alat", 0]}},
 ...
 "video": {"class_type": "CreateVideo", "inputs": {"images": ["vdec", 0], "fps": a.fps, "audio": ["adec", 0]}},
 "save": {"class_type": "SaveVideo", "inputs": {"video": ["video", 0], "filename_prefix": "video/ltx25", "format": "auto", "codec": "auto"}},
}

req = urllib.request.Request(a.server + "/prompt", json.dumps({"prompt": g}).encode(), {"Content-Type": "application/json"})
resp = json.load(urllib.request.urlopen(req))
if resp.get("node_errors"):
    print(json.dumps(resp, indent=1)); raise SystemExit(1)
pid = resp["prompt_id"]
print("queued", pid, f"| {a.width}x{a.height} {a.duration}s@{a.fps}fps seed={a.seed}")
