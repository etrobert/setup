...
Checks that run a program live next to it (`pkgs/<name>/<name>-test.nix`,
fixtures in a sibling `tests/`) and are registered under `checks` in
`pkgs/default.nix`. `checks.yml` discovers check names from the flake, so a new
one becomes its own CI job without touching the workflow or branch protection.

Golden outputs render escape bytes as `<ESC>` so a failing diff stays readable;
regenerate them by rerunning the command under the same `faketime` clock the
check uses.
